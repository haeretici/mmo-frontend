...
`/play` is HuntDL Client chrome on the protocol (P16) plus visual map (P17): three-column layout (runtime · canvas · eq/backpack/combat/skills). No Simulator session form. Mouse default **Classic** (unshifted RMB = default action; Ctrl+click = menu). Click-to-walk sends `MOVE_STEP`. Keyboard walk ignores OS key-repeat: first down immediate, auto-repeat after **200** ms, then interval ≥ `stepMs` (one in-flight `MOVE_STEP`). Bag is `INVENTORY`; equipment slots stay empty until the server sends gear. Death overlay until `STATS` hp > 0. Action bars stay P18.
...
| `static/js/path_walk.js` | orthogonal click-to-walk on the viewport |
